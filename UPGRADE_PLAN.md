...
*Researched 2026-09-16; executed on branch `gunzinger/dep-upgrades`. This branch (`gunzinger/jdk17-floor`) is the **JDK 17 floor variant**: **Gradle 9.7.1** wrapper, **JDK 17** toolchain (artifacts Java 17 bytecode), build/test daemons on **JDK 17**, Xtext matrix 2.38.0 – 2.42.0. The mainline variant with the JDK 21 floor and Xtext 2.43 – 2.44 lives on `gunzinger/dep-upgrades`.*
...
4. **Test matrix (this variant):** `minimumIntegrationTest` = Gradle 8.0 + Xtext 2.38.0; `latestIntegrationTest` = Gradle 9.7.1 + Xtext 2.42.0; both daemons pinned to **JDK 17** via `javaLauncher`.
5. **Phase 3 — Xtext 2.38–2.42 on a JDK 17 floor:** `latestXtextVersion` 2.29.0 → 2.42.0 (build-time Xtend tooling and shipped `org.eclipse.xtend.lib` 2.42), `minimumXtextVersion` 2.17.1 → **2.38.0** — the Xtext span that runs on JRE 17: 2.38 is the first release with Java 17 bytecode, 2.43+ is Java 21 bytecode and cannot even load on a JDK 17 daemon. `options.release` 11 → **17** with toolchain 17 (javac refuses classpath classes newer than the release target), `minimumGradleVersion` stays **8.0** (Gradle 8.0 runs on JDK 17), the JDK 11 min-matrix daemon (`-PminTestJavaHome`) was dropped, and a clear `GradleException` replaced the NPE when `javaSourceLevel` maps to no `JavaVersion` (Xtext 2.38+ dropped pre-Java-8 qualifiers). Ceiling: Xtext ≤ 2.42 `JavaVersion` has no JAVA25, so JDK 22+ daemons are not supported by this variant (use the JDK 21-floor variant on `gunzinger/dep-upgrades` for those).

## End-State Findings (empirical)

- **JDK 17–21 daemons are supported by this variant.** Empirical nuances that the original research (below) got wrong:
    - Xtext **2.38–2.42 is Java 17 bytecode** and runs on JDK 17–21 daemons; 2.43+ is Java 21 bytecode and cannot operate on a JDK 17 daemon, so 2.42 is the newest Xtext usable on this floor.
    - Xtext ≤ 2.42 `JavaVersion` lacks JAVA25 — on JDK 22+ daemons the default `sourceCompatibility` maps to a null `GeneratorConfig.javaSourceVersion` and the builder NPEs, hence the JDK 17 launcher pins.
    - Xtext 2.38+ `JavaVersion.fromQualifier` no longer accepts pre-Java-8 qualifiers ("1.6"/"1.7"); JAVA5–8 all share the qualifiers `1.8`/`8`. Fixtures using obsolete levels were updated, and the builder now fails with a clear error instead of an NPE.
