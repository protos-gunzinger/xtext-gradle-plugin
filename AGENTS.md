...
- The build runs on **JDK 21 or 25** daemons (required: the shipped Xtext 2.44 libs are Java 21 bytecode, so artifacts are Java 21 bytecode via `options.release`). Integration test daemons are pinned via `javaLauncher`: **JDK 21** for `minimumIntegrationTest` (Gradle 8.5 cannot run on newer JDKs), **JDK 25** for `latestIntegrationTest`.
...
3. **Version inference** (`LazyXtextVersion` in `XtextBuilderPlugin`): explicit `xtext.version` → regex detection from classpath jar names → `xtextLanguages` configuration. The hard minimum Xtext version is duplicated in `XtextBuilderPlugin` (string literal `"2.43.0"`) and `gradle.properties` (`minimumXtextVersion`) — update both together. Xtext 2.43+ only supports Java 8+ source levels (`JavaVersion.fromQualifier` returns null for older qualifiers, which fails the build with a clear error).
